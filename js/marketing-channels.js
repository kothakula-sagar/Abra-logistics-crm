...
const MARKETING_SESSION_KEY='abraCRM.marketing.position.v2';
...
function saveMarketingPosition(ch){const s=getState(ch),root=document.getElementById(`view-${ch}marketing`),table=root?.querySelector('.marketing-recipients-scroll')||root?.querySelector('.marketing-campaigns-scroll'),session=readMarketingSession();session[ch]={activeCampaignId:s.activeCampaignId||null,search:s.search||'',tableScrollTop:table?.scrollTop||0,windowScrollY:window.scrollY||window.pageYOffset||0};writeMarketingSession(session)}
function restoreMarketingPosition(ch){const saved=readMarketingSession()[ch];if(!saved)return;const s=getState(ch);if(saved.activeCampaignId&&s.campaigns.some(c=>c.id===saved.activeCampaignId))s.activeCampaignId=saved.activeCampaignId;if(typeof saved.search==='string')s.search=saved.search;requestAnimationFrame(()=>requestAnimationFrame(()=>{const root=document.getElementById(`view-${ch}marketing`),table=root?.querySelector('.marketing-recipients-scroll')||root?.querySelector('.marketing-campaigns-scroll');if(table&&Number.isFinite(saved.tableScrollTop))table.scrollTop=saved.tableScrollTop;if(Number.isFinite(saved.windowScrollY))window.scrollTo({top:saved.windowScrollY,behavior:'auto'})}))}
