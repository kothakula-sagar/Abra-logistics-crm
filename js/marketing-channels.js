...
let contactCacheLoaded=false,contactLoadPromise=null,emailCampaignUnsubscribe=null,whatsappCampaignUnsubscribe=null;
...
const statusOf=(c,ch)=>{const x=ch==='whatsapp'?c.whatsappStatus:c.emailStatus;if(x)return ['Not Interested','Unsubscribed'].includes(x)?'Unsubscribed':'Subscribed';if(c.marketingStatus)return ['Not Interested','Unsubscribed'].includes(c.marketingStatus)?'Unsubscribed':'Subscribed';return 'Subscribed'};
...
async function loadContacts(){
 ...
 contactLoadPromise=contactsRef().orderBy('createdAt','asc').get().then(s=>{const all=s.docs.map(d=>({id:d.id,...d.data()}));state.email.contacts=all;state.whatsapp.contacts=all;contactCacheLoaded=true;contactLoadPromise=null}).catch(e=>{contactLoadPromise=null;console.error(e);toast?.('Failed to load marketing contacts.','danger')});
 return contactLoadPromise;
}
async function refreshContacts(){contactCacheLoaded=false;await loadContacts();renderChannel('email');renderChannel('whatsapp');if(window.Customers)window.Customers.render()}
function subscribeChannel(ch){const ref=getCampaignRef(ch),cb=s=>{state[ch].campaigns=s.docs.map(d=>({id:d.id,...d.data()}));restoreMarketingCampaign(ch);renderChannel(ch)};const old=ch==='email'?emailCampaignUnsubscribe:whatsappCampaignUnsubscribe;if(old)old();const unsub=ref.orderBy('createdAt','desc').onSnapshot(cb,e=>{console.error(e);toast?.(`Failed to load ${ch} campaigns.`,'danger')});if(ch==='email')emailCampaignUnsubscribe=unsub;else whatsappCampaignUnsubscribe=unsub}
...
function setSearch(ch,v){state[ch].search=v||'';renderChannel(ch)}
function closeCampaign(ch){state[ch].activeCampaignId=null;state[ch].search='';clearMarketingCampaignState(ch);renderChannel(ch)}
function openCampaign(ch,id){state[ch].activeCampaignId=id;state[ch].search='';try{sessionStorage.setItem(`crmMarketingActiveCampaign_${ch}`,id);sessionStorage.setItem(`crmMarketingScroll_${ch}`,JSON.stringify({inner:0,outer:window.scrollY||0}))}catch(e){}renderChannel(ch)}
...
saveMarketingScrollState(ch);if(ch==='email'&&campaign.emailProvider==='Outlook')window.location.href=url;else window.open(url,'_blank','noopener,noreferrer');
const sentRecord={openedAt:new Date(),sentThrough:ch==='email'?(campaign.emailProvider==='Outlook'?'Outlook App':'Gmail'):'WhatsApp',sentBy:CURRENT_USER.uid,sentByName:CURRENT_USER.name||CURRENT_USER.email};campaign.sentRecipients={...(campaign.sentRecipients||{}),[contactId]:sentRecord};renderChannel(ch);if(ch==='whatsapp'&&getWhatsAppCooldown(campaign).remaining>0)startWhatsAppCountdown();try{await getCampaignRef(ch).doc(campaignId).update({[`sentRecipients.${contactId}`]:{openedAt:firebase.firestore.FieldValue.serverTimestamp(),sentThrough:sentRecord.sentThrough,sentBy:sentRecord.sentBy,sentByName:sentRecord.sentByName}})}catch(e){console.error('Failed to record campaign open',e)}if(options.shortcutIndex)toast?.(`Lead no ${options.shortcutIndex}, customer name ${c.name} is opened`,'success')}
...
window.MarketingChannels={openView,setSearch,closeCampaign,openCampaign,openContactModal,saveContact,deleteContact,openCampaignModal,saveCampaign,deleteCampaign,openMessage,syncLoadedLeads,syncLeadUpdate,syncExistingLeads,refreshContacts,getContacts:()=>state.email.contacts};
})();
