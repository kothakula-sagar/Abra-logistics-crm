...
(function(){
  ...
  const channelStatus=c=>{if(!String(c?.email||'').trim())return null;const v=c.emailStatus||c.marketingStatus;if(v)return ['Not Interested','Unsubscribed'].includes(v)?'Unsubscribed':'Subscribed';return 'Subscribed'};

  function loadedData(){
    const m=window.MarketingChannels?.getReportData?.()||{customers:[],emailCampaigns:[]};
    ...
    const campaigns=[
      ...emailCampaigns.map(c=>({channel:'Email',name:c.name||'Email Campaign',count:Object.keys(c.sentRecipients||{}).length,eligible:customers.filter(x=>channelStatus(x)==='Subscribed').length,createdAt:c.createdAt})),

    ].sort((a,b)=>dt(b.createdAt)-dt(a.createdAt));
    const interestedCustomers=customers.filter(c=>channelStatus(c)==='Subscribed');
    ...
    return {customers,emailCampaigns,leads,campaigns,interestedCustomers,emailOpened,follows};
  }
  ...
  function render(){
    ...
    root.innerHTML=`
      ...
      <div class="super-kpi-grid">
        ...
        ${metric('Interested Customers',d.interestedCustomers.length,'Email subscribed','💚','customers')}
        ...
        ${metric('Campaigns Created Today',d.campaigns.filter(c=>typeof reportDateKey==='function'&&reportDateKey(c.createdAt)===reportNowDateKey()).length,'Email','📣','report')}
      </div>
      <div class="row g-3 mt-1">
        <div class="col-lg-7"><div class="marketing-card h-100"><div class="marketing-card-title mb-3">Campaign Performance</div><div class="small text-muted mb-3">Messages initiated from CRM versus eligible recipients.</div><div class="dashboard-campaign-list">${d.campaigns.length?d.campaigns.slice(0,20).map(c=>{const pct=c.eligible?Math.min(100,Math.round(c.count/c.eligible*100)):0;return `<div class="dashboard-campaign-item mb-3"><div class="d-flex justify-content-between gap-2 mb-1"><div><span class="badge bg-primary-subtle text-primary me-2">${c.channel}</span><strong>${esc(c.name)}</strong></div><span class="small fw-semibold">${c.count}/${c.eligible}</span></div><div class="progress report-progress"><div class="progress-bar bg-primary" style="width:${pct}%"></div></div><div class="small text-muted mt-1">${pct}% of eligible recipients initiated</div></div>`}).join(''):'<div class="text-center text-muted py-4">No marketing campaigns loaded yet.</div>'}</div></div></div>
